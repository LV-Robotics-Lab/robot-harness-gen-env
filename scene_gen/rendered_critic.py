...
def build_critic_prompt(resolved: ResolvedSceneSpec) -> str:
    ...
    contract = {
        "task": (
            "Judge the rendered RoboTwin scene against the requested scene."
            " Use only visible evidence. Static geometry and runtime checks"
        ...
        "rules": [
            ...
            (
                "Return exactly five check objects, one for every required_chec"
                "ks entry; never omit a check."
            ),
            (
                "Use the exact required check names and use status not_applicab"
                "le when a check does not apply."
            ),
            (
                "For articulation_state, use not_applicable only when no articu"
                "lated state was requested."
            ),
    ...
    *,
    ...
    contract = {
        ...
        "rules": [
            ...
            (
                "Judge this check from the rendered views and request without a"
                "ssuming hidden geometry."
            ),
...
def _normalize_checks(parsed: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for check in checks:
        ...
        if name not in REQUIRED_CHECKS or status not in {
            "pass",
            "fail",
            "warning",
            "not_applicable",
...
def review_rendered_scene(
    *,
    ...
    base = {
        ...
        "images": [{"path": str(path), "sha256": _sha256(path)} for path in existing],
    ...
    raw = inference(
        image_paths=existing, prompt=build_critic_prompt(resolved), model_name=model_name
    )
    ...
    for issue in issues:
        ...
        if severity not in {"blocker", "major", "minor"} or not target or not message:
            continue
        normalized_issues.append({"severity": severity, "target": target, "message": message})
    ...
    response_statuses = [str(response.get("status") or "").lower() for response in parsed_responses]
    invalid_statuses = [
        value or "missing" for value in response_statuses if value not in {"pass", "fail"}
    ]
    if invalid_statuses:
        normalized_issues.append(
            {
            ...
            normalized_issues.append(
                {
                    ...
                    "target": "critic_contract",
                    "message": (
                        "articulation_state is required but the model returned not_applicable"
                    ),
                }
            )
    failed = any(item["status"] == "fail" for item in normalized_checks)
    blocking_issue = any(item["severity"] in {"blocker", "major"} for item in normalized_issues)
    status = (
        "pass"
