...
def generated_module_source(resolved: ResolvedSceneSpec) -> str:
    ...
    return (
        '"""Generated by scene_gen.builder from a validated ResolvedSce'
        'neSpec."""\n\nimport json\n\nfrom scene_gen.envs.generated_scene i'
        "mport load_resolved_scene\n\nRESOLVED_SCENE_JSON = "
        f"{payload!r}"
        "\nRESOLVED_SCENE_SHA256 = "
        f"{resolved.digest()!r}"
        "\n\ndef load_scene(task, resolved_scene=None):\n    payload = res"
        "olved_scene if resolved_scene is not None else json.loads(RESO"
