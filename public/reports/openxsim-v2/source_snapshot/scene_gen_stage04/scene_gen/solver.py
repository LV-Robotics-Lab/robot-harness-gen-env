...
COMPILER_VERSION = "scene_gen.stage2_solver.v1"
...
def _candidate_reasons(
    ...
    for other_id, other in assigned.items():
        ...
        if distance < candidate.radius + other.radius + 0.01:
            reasons.append(f"footprint overlap with {other_id}")
...
def solve_scene(
    ...
    *,
    ...
    order = sorted(spec.objects, key=lambda item: (-degree[item.object_id], item.object_id))
    ...
    def place(index: int) -> bool:
        ...
        for _ in range(max_attempts_per_object):
            ...
            if x_min >= x_max or y_min >= y_max:
                reasons = ["asset footprint does not fit selected region"]
                x = (region_x[0] + region_x[1]) / 2.0
                y = (region_y[0] + region_y[1]) / 2.0
            else:
                x = rng.uniform(x_min, x_max)
                y = rng.uniform(y_min, y_max)
                candidate = CandidatePose(x=x, y=y, yaw=yaw, half_x=half_x, half_y=half_y, radius=radius)
                reasons = _candidate_reasons(query.object_id, candidate, assigned, spec.relations, spec.workspace)
            candidate = CandidatePose(x=x, y=y, yaw=yaw, half_x=half_x, half_y=half_y, radius=radius)
            attempts.append(
                SolverAttempt(
    ...
    for query in sorted(spec.objects, key=lambda item: item.object_id):
        ...
        z = spec.workspace.table_height_m
        if model.z_policy == "center_on_table":
            z += model.dimensions_m[2] / 2.0
        files = tuple(
            sorted(
                {
                    value
        ...
        resolved_objects.append(
            ResolvedObject(
                ...
                pose=ResolvedPose(
                    position_m=(round(pose.x, 9), round(pose.y, 9), round(z, 9)),
