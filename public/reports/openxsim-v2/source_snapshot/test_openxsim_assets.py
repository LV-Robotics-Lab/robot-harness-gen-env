...
def test_obj_preview_renders_source_and_normalized_geometry(asset_server, tmp_path: Path) -> None:
    _, base_url = asset_server
    candidate = AssetScout([CatalogSearchProvider(f"{base_url}/catalog.json")]).search("mug")[0]
    downloaded = download_candidate(candidate, tmp_path / "cache")
    bundle = compile_downloaded_asset(downloaded, tmp_path / "compiled", asset_id="preview_mug")
    normalized = next(item for item in bundle.representations if item.format == "obj")
    source_preview = render_obj_preview(downloaded.path, tmp_path / "source.png", size_px=256)
    imported_preview = render_obj_preview(normalized.uri, tmp_path / "imported.png", size_px=256)

    assert Path(source_preview["path"]).is_file()
    assert Path(imported_preview["path"]).is_file()
    assert source_preview["vertex_count"] == imported_preview["vertex_count"]
    assert source_preview["source"] != imported_preview["source"]
