...
## Expanded acceptance (2026-07-16)

The expanded implementation gates are reproducible with:

```bash
MUJOCO_GL=cgl .venv/bin/python scripts/run_anchor2env_acceptance.py \
  --output artifacts/openxsim_acceptance/anchor2env

.venv/bin/python scripts/run_asset_scout_acceptance.py \
  --catalog configs/openxsim/asset_sources_v1.json \
  --output artifacts/openxsim_acceptance/asset_scout

MUJOCO_GL=cgl .venv/bin/python scripts/run_crosssim_acceptance.py \
  --output artifacts/openxsim_acceptance/crosssim
```

Observed bounded results:

- Anchor2Env passes three deterministic scenes: two still images and one video.
  The video uses eight distinct sampled frames, and the report preserves
  text-only versus anchor-conditioned geometry changes plus three expected
  failure cases. All three packages also load and step for 120 frames in
  SAPIEN; the generic smoke runner does not bind `robotwin_primitive_plan`.
- AssetScout passes ten rigid meshes from three pinned official repositories.
  Every asset preserves source and imported previews, provenance, hashes,
  visual and collision representations, and generated backend formats. All ten
  pass MuJoCo runtime import; all ten separately pass SAPIEN ground-contact,
  settling, finite-position, and non-empty-render checks.
- Cross-simulator reuse passes L0-L3 for a rigid drop, a one-DOF hinged cabinet,
  and a camera-sensitive red/blue marker scene from one versioned package per
  case. The hinge trajectory maximum error is 0 rad. The normalized red and
  blue image-centroid errors are approximately 0.003 at 640x480 and 55-degree
  vertical FOV.
- The rigid-drop maximum position error is 0.0194 m. MuJoCo has a four-step
  transient rebound gap that SAPIEN does not; the acceptance declares a
  five-step (50 ms) contact-gap debounce and reports both the four raw mismatch
  steps and zero post-debounce mismatch steps.

These results do not promote L4 policy behavior. MetaSim remains
`adapter_only` until its runtime dependencies and matching controller assets are
selected for the same validation host. No seamless simulator-equivalence claim
is made.
