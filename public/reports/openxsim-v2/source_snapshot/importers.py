...
def _mjcf_articulation_tree(root_body: ET.Element) -> dict[str, Any] | None:
    bodies = list(root_body.iter("body"))
    movable_children = [body for body in bodies[1:] if body.find("joint") is not None]
    if not movable_children:
        return None
    if any((body.find("geom") is None or body.find("geom").get("type", "sphere") != "box") for body in bodies):
        return None

    links: list[dict[str, Any]] = []
    joints: list[dict[str, Any]] = []
    root_link = _valid_identifier(root_body.get("name") or "root_link", "link")

    def visit(body: ET.Element, parent_link: str | None, *, root: bool = False) -> None:
        link_id = root_link if root else _valid_identifier(body.get("name") or f"link_{len(links)}", "link")
        geom = body.find("geom")
        ...
        links.append(
            {
                ...
                "pose": {
                    ...
                    if root
                    else list(_floats(body.get("pos"), 3, (0.0, 0.0, 0.0))),
                    "orientation_wxyz": [1.0, 0.0, 0.0, 0.0]
                    if root
                    else list(_floats(body.get("quat"), 4, (1.0, 0.0, 0.0, 0.0))),
                },
                "geometry": {
                    "type": "box",
                    "half_size_m": list(_floats(geom.get("size"), 3, (0.05, 0.05, 0.05))),
                    "color_rgb": list(_floats(geom.get("rgba"), 4, (0.65, 0.68, 0.72, 1.0)))[:3],
                },
                "geometry_origin": {
                    "position": list(_floats(geom.get("pos"), 3, (0.0, 0.0, 0.0))),
                    "orientation_wxyz": list(_floats(geom.get("quat"), 4, (1.0, 0.0, 0.0, 0.0))),
                },
                "mass_kg": float(geom.get("mass") or 0.2),
            }
        )
        ...
        if parent_link is not None:
            if joint is None:
                joints.append(
                    {"id": f"{parent_link}_to_{link_id}", "type": "fixed", "parent": parent_link, "child": link_id}
                )
            else:
                mjcf_type = str(joint.get("type") or "hinge")
                joint_type = "revolute" if mjcf_type == "hinge" else "prismatic" if mjcf_type == "slide" else mjcf_type
                limits = list(_floats(joint.get("range"), 2, (-1.57, 1.57)))
                joints.append(
                    {
                        "id": _valid_identifier(joint.get("name") or f"{parent_link}_to_{link_id}", "joint"),
                        "type": joint_type,
                        "parent": parent_link,
                        "child": link_id,
                        "axis": list(_floats(joint.get("axis"), 3, (0.0, 0.0, 1.0))),
                        "range": limits,
                        "damping": float(joint.get("damping") or 0.05),
                    }
                )
        for child in body.findall("body"):
            visit(child, link_id)

    visit(root_body, None, root=True)
    return {
...
def import_mjcf(path: str | Path, *, package_id: str | None = None) -> EnvironmentPackage:
    ...
    if worldbody is not None:
        for top_index, top_body in enumerate(worldbody.findall("body")):
            articulation = _mjcf_articulation_tree(top_body)
            if articulation is not None:
                name = _valid_identifier(top_body.get("name") or f"body_{top_index}", "body")
                asset_id = _valid_identifier(f"{name}_asset", "asset")
                assets.append(
                    AssetBundle(
                        asset_id=asset_id,
                        category="articulated_object",
                        representations=(
                            AssetRepresentation(format="mjcf", uri=str(source), backend="mujoco", role="source"),
                        ),
                        source={"kind": "mjcf_articulation", "path": str(source), "root_body": name},
                        articulation=articulation,
                        tags=("articulated", "native_mjcf"),
                    )
                )
                ...
                objects.append(
                    SceneObject(
                        ...
                        asset_id=asset_id,
                        pose=Pose(
                            position=_floats(top_body.get("pos"), 3, (0.0, 0.0, 0.0)),
                            orientation_wxyz=_floats(top_body.get("quat"), 4, (1.0, 0.0, 0.0, 0.0)),
                        ),
                        static=top_body.find("freejoint") is None and top_body.find("joint") is None,
                        metadata={"source_body": top_body.get("name", ""), "articulation_preserved": True},
                    )
                )
                continue

            for nested_index, body in enumerate(top_body.iter("body")):
                name = _valid_identifier(body.get("name") or f"body_{top_index}_{nested_index}", "body")
                geom = body.find("geom")
                if geom is None:
                    continue
                asset_id = _valid_identifier(f"{name}_asset", "asset")
    ...
    if sensors_custom is not None and sensors_custom.get("data"):
        try:
            raw_sensors = json.loads(sensors_custom.get("data", "[]"))
            if isinstance(raw_sensors, list):
                sensors = tuple(dict(item) for item in raw_sensors if isinstance(item, dict))
        except json.JSONDecodeError:
            sensors = ()
    if not sensors and worldbody is not None:
        sensors = tuple(
            {
                "id": _valid_identifier(camera.get("name") or f"camera_{index}", "camera"),
                "type": "camera",
                "position": list(_floats(camera.get("pos"), 3, (0.0, -1.0, 1.0))),
                "orientation_wxyz": list(_floats(camera.get("quat"), 4, (1.0, 0.0, 0.0, 0.0))),
                "fov_y_deg": float(camera.get("fovy") or 60.0),
                "source": "native_mjcf_camera",
            }
            for index, camera in enumerate(worldbody.findall("camera"))
        )
    if contract:
        task = TaskSpec(
    ...
    package = EnvironmentPackage(
        ...
        env=EnvSpec(
            ...
            metadata={
                "mjcf_angle": angle,
                "articulated_object_count": sum(bool(asset.articulation) for asset in assets),
