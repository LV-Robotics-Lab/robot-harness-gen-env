...
| default ACT remains unpromoted and default wrappers are not wired | fixed-placement ACT passes 3/3, but two signature-disjoint varied-placement ACT evals each score 1/4; `runs/policy_train_eval_entrypoint_probe/probe_report.json` records old data/config/module assumptions; a separate privileged pose-conditioned baseline passes 4/4 held-out, 4/4 declared domain randomization, and 3/3 fixed-placement can/basket | the bounded pose-conditioned baseline can be used only inside its declared gate; default ACT repair and visual/language-conditioned robustness remain separate post-TODO work |
...
The remaining generation2env gap is broad generated-asset import and physics validation: scale, collision, articulation, materials, simulator load, and task verifier coverage. On the policy side, the privileged pose-conditioned baseline passes its bounded SceneAgent gate, while visual/language-conditioned closed-loop robustness and default ACT wrapper compatibility remain unproven. The two held-out ACT splits remain 1/4 and are retained as negative evidence rather than being overwritten by the separate baseline.
