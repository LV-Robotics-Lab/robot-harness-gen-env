...
def fuse_anchor(package: EnvironmentPackage, anchor: AnchorSpec) -> EnvironmentPackage:
    ...
    for obj in package.env.objects:
        ...
        if constraint and "position_m" in constraint:
            position = tuple(float(value) for value in constraint["position_m"])
            pose = Pose(position=position, orientation_wxyz=obj.pose.orientation_wxyz)
            metadata = {
                **obj.metadata,
                "anchor_override": {
                    "anchor_id": anchor.anchor_id,
                    "confidence": float(constraint.get("confidence", anchor.confidence)),
    ...
    env = replace(
        ...
        metadata={
            **package.env.metadata,
            "anchor_fusion": {
                "anchor_id": anchor.anchor_id,
                "applied_object_positions": sorted(object_updates),
