...
def test_single_workflow_advances_from_model_to_resolver_and_compile(tmp_path, missing_width):
    ...
    assert [op.capability for op in snapshot.operations] == [
        "ingest",
        "codex.interpret",
        "asset.resolve",
        "x2env.compile",
    ...
    assert snapshot.required_resources == ("genesis_replay_executor",)
    assert harness.resume(handle.workflow_id) == snapshot
    assert not (tmp_path / "package").exists()
