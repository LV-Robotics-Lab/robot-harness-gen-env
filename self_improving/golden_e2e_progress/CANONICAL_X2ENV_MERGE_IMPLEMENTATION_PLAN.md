...
## 当前优先授权：2026-09-13 两小时收口窗口（matrix v2）

本节优先于下文历史 v1 的数量、覆盖率及运行预算门；未冲突的真实性、许可、不可变证据、
Harness 唯一权威与正常 push/main PR 边界保持。起点为 `2026-09-13T06:25:09Z`，
硬截止为 `2026-09-13T08:25:09Z`；准备、执行、审阅及收口均计入 7200 秒，不因模型等待重置。
按 [qualification-matrix-v2.json](qualification-matrix-v2.json) 执行四个简单真实案例：
S01 简单粉红鼠标文本；S02 原图且无 text；S03 原视频且无 text；S04 原图加蓝方块文本。
四例采用默认资产策略，不能继承旧 local-only/web-only 强制策略；不得暗加布局、尺寸或模型 proposal。

本窗口阻断条件为四例真实成功及至少一个本窗口成功环境包的新目录真实 copy-run；
每项仍必须有真实物理、视觉、媒体、闭包和独立运行证据，不能把 preview 或历史成功换名计入。
每次 case 总上限 1200 秒（含退出清理）；外层同时受剩余全局时间约束。
总分 ≥8/12、三来源各成功、三个不同包 copy-run、core 语句/分支 100% 改为非阻断测量项，
如实列出已测分母、失败、未运行及覆盖缺口，不能写成达标或降低单例物理阈值。
复杂场景和动态支撑不再是本窗口实现前置；已提交实现与 v1 矩阵保留，不追认历史资格。
截止必须停止新增实验并收口已完成产物及阻断，不以继续补齐历史门延长窗口。

以下原计划保留为历史完整范围；本窗口执行与门禁以本节及 v2 为准。
