...
- [x] 计划与公开只读部署检查，24项CLI/配置测试通过。
- [x] 当前部署配置 web 和固定重建后端；轻量 check 通过，不代替真实运行。
- [x] web 首例实际搜索下载，发现源轴建议错误，TDD修复；失败保留。
- [ ] 修复后 web、local miss→web、真实新几何的完整案例与分阶段测量。
- [ ] 文档/来源/结果收口、定向及根测试、提交与 bingsheng push。

## 2026-09-13 后续授权：根目录恢复与运行路径迁移

- [x] 14 项实体迁移、259 个兼容链接撤销，历史字节和 refs 保留；当前入口统一至 runtime/harness。
- [x] 固定源码、当前配置、本地资产/原媒体核验；真实 Genesis 25 步及新媒体通过。
- [x] 当前指南更新，历史报告按路径映射读取；不重写历史证据，不新增 push/main PR。

## 2026-09-13 Push 后目录收尾（另行授权）

- [x] 目录、进程、Git、未合并提交和入口依赖逐项盘点。
- [x] 证据/备份集中归档，旧路径兼容链接核验。
- [x] 86 个非必要工作树完整快照后退役，dirty 和依赖工作树保留。
- [x] 入口静态检查、媒体/证据 SHA 和 refs 核验，恢复说明及 repo-docs 同步。

该 housekeeping 不授予新的运行能力；详见本机 archive/2026-09-13-workspace/audit。
下文旧阶段验收以当时范围保留；实际 matrix v2 push 结果见最终外部审计。
