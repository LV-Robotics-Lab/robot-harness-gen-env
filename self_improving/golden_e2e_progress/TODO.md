...
- [ ] 新窗口 S01：首轮179.63s颜色审核失败保留；修复后da78d40第二轮已compile、真实replay运行，未计成功。
- [ ] 新窗口 S02：50.625s部署拒读目录错误、51.226s字段路径误澄清均保留；修复后1228f96第三轮运行。
- [ ] 新窗口 S03：原视频无text、seed37、默认资产策略；da78d40已compile、真实replay运行，未计成功。
- [ ] 新窗口 S04：原图+蓝方块文字覆盖、seed41、默认策略；1228f96新独立调用运行，未计成功。
- [ ] 至少一个本窗口成功包的新目录隔离 copy-run、新媒体及检查报告。
