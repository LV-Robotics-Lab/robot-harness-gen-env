...
- [ ] P1-reconstruction：3次真实新几何/登记/预览，仍未完整成功；最新重复几何证据超预算，正在修复。
...
- [x] root 2411 passed/1 skipped（281.08秒）；后续颜色切片单独测量，不称同源冻结资格。
- [ ] 后续失败收口：web许可/材质/语义范围；source receipt失败聚合；模型澄清具体原因的用户可读表达。

## 当前：根目录收敛与运行路径迁移

- [x] 保存原配置/说明与 Git 清单，逐项移动 14 个实体目标，撤除 259 个已核对链接。
- [x] 修复 Git 登记及当前配置，核验 202 个 refs、固定源码、原媒体和本地资产目录。
- [x] 迁移后小型真实 Genesis load/step/新输出核验：25 步、60.35 秒，拒读六个原来源根。
- [x] 更新当前导航、路径映射及 repo-docs，独立本地提交；不 push。
...
`2026-09-13T06:25:09Z` 开始、`08:25:09Z` 硬截止；每 case 含退出最多 20 分钟。

- [x] 新窗口 S01：da78d40第二轮339.200397s succeeded，同workflow真实视觉/物理/交付通过；首轮失败保留。
- [x] 新窗口 S02：e754ff9第五轮290.985332s succeeded；前四次失败保留。
- [x] 新窗口 S03：e754ff9第三轮312.627s succeeded，原视频无text/seed37/default；前两次失败保留。
- [x] 新窗口 S04：9d5909b第四轮383.564632s succeeded、蓝child/视觉/物理/交付通过，前三次失败保留。
- [x] S01本窗口成功包复制至独立目录；真实双dt125.649183s、26物理项及9原根拒读通过，新连续媒体保留。
- [ ] 收口每阶段耗时/失败/部分产物/模型证据/进程退出，保留历史结果；到硬截止停止新增执行。
- [ ] 测量但不阻断：≥8/12、三来源成功、三个 copy、core 100%；列清分母与未运行，不追认完成。
- [x] e6570f3六组active测试+coverage merge通过；core7522/8179语句、2838/3368分支，未扩大排除。
  后续改动按增量测试记录，最终审计须写明适用源码，不能把本条称最终HEAD全量验证。
- [x] e755421六组+独立root+merge全通过751.47s；core7552/8208语句、2848/3378分支。
- [x] e754ff9六组/root/merge通过802.83s，root2398 passed/1 skipped；覆盖完整缺口保留。
- [ ] 最后冻结源码含坏JSON导出/格式重生成及收尾文档，重新同源active测试和Git审计；
  最终外部记录固定为`/home/jingxiang/bingsheng/canonical-matrix-v2-push-audit-20260913.cKTw0K/push-audit.json`。
  冻结后不改此TODO伪造自包含身份；该最终记录是剩余门的执行结果权威。
- [x] 新路由真实受管理interpret接通、颜色schema后缀审核及合法entity-ID未知路径修复；不改max或模型。
- [x] 用户入口/安装配置/四模态/状态与恢复边界指南已提交；具体成功包与可复用部署入口待收尾补齐。
- [x] 独立用户deployment/两父资产登记/USAGE与非法输入真实检查已就绪；未预填proposal或复用旧state。
- [x] [本轮walkthrough及四例索引](CANONICAL_MATRIX_V2_PUSH_AUDIT_20260913.md)已收口；4/4不自动授权push。

下表为窗口开始前的组件/历史案例基线，不替代上述新窗口通过记录。
动态支撑与复杂场景停止扩展；历史覆盖率缺口不再阻断本窗口。

| 项目 | 状态 | 下一动作或准确范围 |
|---|---|---|
| S01同workflow与隔离复制 | 真实通过（开发子集） | 原包无reset；不升级最终资格 |
| 初始reset/实际拓扑 | 真实通过（operator双dt小测） | visual、步进后恢复未验，不是S02 |
| S02 image-only | 受阻 | 固定b2b63b7本次interpret 600s超时；不原样重试。独立PBR真实红色探针58.04s通过，不计S02；S03已放行一次原定提交 |
...
| v3动态布局 | 集成接通（采纳前/完成门） | 共享只读独立审核及失真后端拒绝通过；真实模型动态案例待补 |
...
| 用户媒体许可 | 合同或设计 | 已获CC-BY-4.0、署名x2env1.0；仅指定媒体及派生资产 |
| dashboard同步 | 受阻 | 三入口404，无task id，不伪造更新 |

当前权威：canonical整合worktree；每个agent独占文件，主线程管理集成/文档。
准确运行路径、耗时、失败、hash与各独立测试测量见RESULTS；下文只保留逐次执行历史，不是当前待办。
...
- S02 corrected31osM1一次真实提交229.50s后blocked：模型序号unknown路径未被旧classifier解析；原失败保留，v2 agent补严格路径映射后再固定新运行。
...
- C08/C09增量契约已落盘：先固定Genesis实际拓扑，再实测面/动态支撑和多实体设计v2；首片禁止只删类型门。状态仍合同或设计。

- S01 fixed a57a0ea同run真实闭环succeeded，546.60s，26/26物理和最终颜色视觉通过；包仍开发未资格。
  下一步其包有界隔离copy-run；S02/S03/S04/M04继续，动态刚体支撑/多实体grounding须补足>=8/12。
- 当前canonical完整目录1028项通过/149.20s；六组同源覆盖100%尚未补齐。dashboard三入口复查仍404。

- web解析45项通过，尚缺相邻时钟预算1语句/1分支；S01同run颜色child修改及视觉复验成功，当前grounding阶段，最终资格未授予。

- 两份冻结用户媒体及派生重建已获CC-BY-4.0发布授权，署名x2env1.0；运行adapter需消费固定哈希授权记录，发布仍等验收。

- 部署视频/重建阻断测试38项通过，deployment仍缺8语句/4分支；完整测试/矩阵/推送条件仍未满足。

- a57a0ea S01真实interpret已完成并进入本地候选Genesis预览，尚无最终场景；生命周期19项通过，旧pipeline缺值预期已同步早期澄清。

- package公开攻击32项通过，无生产改动；缺loader真实失败已留证，不计成功copy-run。新版a57a0ea单次真实S01已分配。
...
- assessment攻击59项通过，279/281语句、136/138分支；未到100%，不授仿真资格。

- 固定5fe0225新树六组/root/归并验证已分配，专用CI环境与本地固定子模块，不混入旧测量。
- interpret提示精简作为单独性能实验：保留全部字段、出处和unknown，max不变；先测试/审阅提交，后一次有界真实case，不预承诺消除timeout。
- assessment公共seam攻击测试独占补齐；主线程审阅，组件覆盖不授予真实仿真能力。dashboard三入口仍404，未更新远端任务。
...
- 请求绑定模型schema已修真实哈希抄错，104项主线程回归通过；固定新版本后单次验证S01，超时改善未证。
- Yuxin范围915项通过、2项缺SAPIEN跳过，lint清零；历史16链接原文归档可读，完整文档门实现中。
- dashboard三个入口再次HTTP404，无task id；未声称同步状态。
