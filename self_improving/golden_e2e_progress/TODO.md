...
| S02 image-only | 受阻 | 保留229.50s失败；修严格序号字段映射和灰暗材质后固定新case |
| v2多实体生成布局 | 组件实现 | 独立controller授权与transport完成门检查；主线程待最终审阅/提交 |
| 实测动态支撑 | 合同或设计 | 明确面域/孔洞/逐帧target合同，再TDD；不删类型门冒充实现 |
| 三输入/三来源及12例矩阵 | 集成接通（有限子集） | S02/S03/S04/M04及复杂支撑案例继续；尚无4/4或≥8/12 |
| 语句/分支覆盖门 | 受阻 | 六组旧基线90.46%/80.03%；新单文件结果不能混并 |
