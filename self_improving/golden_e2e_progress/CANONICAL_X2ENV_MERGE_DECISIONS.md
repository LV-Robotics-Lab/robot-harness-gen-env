...
## 2026-09-13 最新用户授权：Q65 两小时简单四例收口

- 生效窗口：`06:25:09Z` 至 `08:25:09Z`，同日 UTC；单次 case 含清理最多 20 分钟。
- S01 精确文本为“在桌面放一个粉红色鼠标。”；S02 原图无文本；S03 原视频无文本，
  仅恢复静态环境是 case purpose，不注入用户 text；S04 精确文本为
  “参考图片，在桌面放一个蓝色方块，颜色以文字要求为准。”并附原图。
- 采用默认资产策略，仍由 Harness 固定解析顺序和许可门，不为案例预选来源或伪造命中。
- 四简单案例真实成功及至少一个本窗口成功包 copy-run 仍为阻断门。
  ≥8/12、三来源成功、三个不同包 copy-run、core 100% 改为非阻断但必须测量和报告。
- 本条覆盖 Q32–Q35 等历史数量/来源门及旧 30 分钟单次预算的冲突部分；不改变物理阈值、
  视觉独立判断、证据完整性、无模型预填、开发与正式资格区分、禁止未经授权 main PR。
