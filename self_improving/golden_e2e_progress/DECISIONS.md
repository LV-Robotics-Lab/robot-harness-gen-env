...
## 2026-09-13 — 结构表面颜色的数值表达

- SceneEntity.surface_rgba仅供结构支撑，0..1 RGBA与原color/provenance共存；原色名不替换为数字字符串。
- Codex提供估计而非测量；foreground仍使用资产规范化/修订，不允许此字段覆盖资产材质。
- 字段缺省不改变历史SceneIR序列化，旧可解析颜色路径保留；未知颜色且未给数值继续失败。

## 2026-09-13 — 检索修订与意图修订分离

- 词法检索修订只改变查找描述，不改变SceneIR/category、许可或视觉判据。最多一次、共享asset.resolve原时限。
- 仅无候选/候选视觉失配/明确不支持格式可触发；外部资源、许可、证据错误或超时停止。
- 两次来源结果和模型回执均保留；重复query停止，重复candidate不再下载/预览。不新增Skill或workflow。

## 2026-09-13 — 大几何证据存储而非验收降级

- 原始运行结果保留全部字段；预览receipt以ArtifactRef引用，不内嵌第二份，运行JSON不做缩进排版。
- CAS仍核原字节/hash，256MiB闭包和引用/节点预算不提高。派生存储对比不修改原失败workflow。
- loaded.json与child结果当前仍含重复拓扑；本切片只消除预览再复制及格式放大，不宣称所有重复已解决。

## 2026-09-13 — 按 roadmap 推进 P0/P1

- 用户已授权创建计划并推进；本轮先完成公开部署检查、web 与 reconstruction 实际部署和有界案例。
- check 是配置诊断，configured 不是 runtime_pass；缺省可选来源与已配置但损坏分开报告。
- 沿用唯一 CLI/controller 和既有资产 adapter。每次核验最多30分钟；不推进 main PR。

## 2026-09-13 — 撤销旧地址兼容并迁移当前运行目录

- 用户明确批准将根目录实验名称全部收敛，并撤销历史兼容链接；此项替代下文“保持原路径”。
- 当前入口与依赖迁入 runtime，历史证据/dirty 文件保留于 archive，分支和 refs 不删除。
- 旧哈希证据不重写，通过本机 archive/2026-09-13-root-layout/path-map.json 查找新路径。
- 当前配置与操作文档更新，小型真实 Genesis 运行核验迁移结果；不重新授予模型/全流程资格。
- 本次本地文档提交不包含密钥或大产物，不新增 push 或 main PR。

## 2026-09-13 — 推送后的文档任务

- 用户授权补全使用/API/测试/接入/roadmap 后再次 commit/push bingsheng；main PR 未授权。
- 当前能力按 matrix v2 和实际代码描述，旧 v1 目标仅作为历史计划；不新增功能或改写运行身份。
- 已有目录整理文档改动保留并一并提交；新增文档从 repo-docs/canonical-x2env.md 导航。

## 2026-09-13 用户另行批准的本地目录收敛

- 只整理本用户范围；主仓库、可用入口和实际运行依赖保持实体原路径。
- 工作树退役不代表分支合并：先保留全部文件和 Git ref，dirty、活动进程或运行依赖不明确时保留。
- 证据和备份集中到本机 archive，保留原路径阅读链接，不改写已哈希报告中的绝对地址。
- 链接不是旧严格部署的执行兼容承诺；恢复实体目录并重新核验后才能重用历史运行。
- 不永久删除证据、分支或未合并提交；不把同盘 hardlink 归档称为异地备份或磁盘空间优化。
- 本次只增加本地整理文档提交，不再次 push 或创建 main PR。

## 2026-09-13 两小时窗口的阻断门与测量项分离

- 用户新授权窗口固定为 06:25:09Z–08:25:09Z；单次 case 含清理最多 1200 秒。
- [matrix v2](qualification-matrix-v2.json) 四个简单输入采用默认资产策略；
  无文本模态不补隐藏用户文本，视频目标仅静态环境。
- 阻断门：四例真实成功、至少一个本次成功包实际隔离 copy-run；真实性和物理阈值不降。
- 非阻断测量：≥8/12、三来源、三 copy、core 100%；缺口仍真实报告，不记完成。
- 旧矩阵/失败/许可/资格不回填。动态支撑与复杂矩阵不延误本窗口；到点收口而非无限继续。
...
- 新初始reset日志与loaded/step0绑定；后者仍须真实独立验证。领域定义见根CONTEXT，不把技术日志字段当领域定义。
