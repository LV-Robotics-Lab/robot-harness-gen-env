...
def test_solver_places_stack_and_container_contents_in_three_dimensions() -> None:
    catalog = _real_catalog()
    stacked = solve_scene(parse_rule_based("Place a red block on top of a plate.", seed=31), catalog)
    stack_objects = {item.object_id: item for item in stacked.objects}
    block = stack_objects["block_1"]
    plate = stack_objects["plate_1"]
    assert block.support_relation == RelationType.ON_TOP_OF
    assert block.support_target == "plate_1"
    assert block.pose.position_m[2] > plate.pose.position_m[2]
