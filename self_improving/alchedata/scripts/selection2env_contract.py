...
def validate_task_program_references(
    ...
    *,
        ...
        checks.append(
            {
                ...
                "status": "fail",
                "message": portable_path(placement_path, root=root),
            }
        )
        return {
            "status": "fail",
            "checks": checks,
            "placement_path": portable_path(placement_path, root=root),
        }
    ...
    expected_relation = TEMPLATE_RELATIONS.get(binding["template"]) if binding else None
    checks.append(
        {
            ...
            "message": (
                ...
                f"expected_relation={expected_relation}"
            ),
        }
    )
    if binding and binding["target_kind"] == "object":
        verifier_target = verifier.get("target_object_id")
        expected_target = binding["target_id"]
        target_matches = verifier_target == expected_target and not verifier.get("target_region")
    elif binding:
        verifier_target = verifier.get("target_region")
        expected_target = binding["target_region"]
        target_matches = verifier_target == expected_target and not verifier.get("target_object_id")
    else:
    ...
    checks.append(
        {
            "name": "verifier_target_matches_binding",
            "status": "pass" if target_matches else "fail",
            "message": f"verifier_target={verifier_target} expected_target={expected_target}",
        }
    )
