...
def build_episode_record(
    ...
    return {
        ...
        "planned_joint_paths": report.get("left_joint_path_len", 0)
        + report.get("right_joint_path_len", 0),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generated selection2env multi-episode rollout collection."
    )
    ...
    parser.add_argument(
        "--seed-list", help="Comma-separated seed list. Overrides --episodes and --seed-start."
    )
    ...
    parser.add_argument(
        "--runner", default=str(ROOT / "scripts" / "run_generated_selection2env_rollout_probe.py")
    )
    ...
    base_report: dict[str, Any] = {
        ...
        "scene_module": str(Path(args.scene_module).expanduser().resolve())
        ...
        if not args.seed_list and not args.placement_manifest
        else None,
        "seed_list": [case["seed"] for case in cases],
        "placement_manifest_status": placement_manifest.get("status")
        if placement_manifest
        else None,
        "status": "started",
    ...
    for index, case in enumerate(cases):
        ...
        if args.record_native_data:
            command.extend(
                ["--record-native-data", "--native-save-freq", str(args.native_save_freq)]
            )
    ...
    def episode_passed(episode: dict[str, Any]) -> bool:
        action_passed = (
            episode["status"] == "pass_generated_action_rollout"
            and episode["check_success"] is True
        )
    ...
    status = (
        "pass_generated_rollout_collection"
        if pass_count == len(episodes)
        else "fail_generated_rollout_collection"
    )
    events.append(
        {
