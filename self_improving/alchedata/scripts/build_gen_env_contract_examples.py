...
def common(task_text: str, route: str, status: str) -> dict[str, Any]:
    return {
        ...
        "inputs": {
            ...
            "asset_catalog_roots": [
                (
                    "external/robotwin-text2env-demo/asset_catalogs/robotwin_tablet"
                    "op_assets_master.json"
                ),
    ...
    record.update(
        {
            "outputs": {
                "selection2env": {
                    ...
                    "scene_task_decoupling": {
                        ...
                        "claim": (
                            "scene_apple_plate_shared_two_task_v0 passes two task specs ove"
                            "r one placement SHA"
                        ),
                    },
            ...
            "blockers": [],
            "claim_boundary": (
                "This sample validates the selection2env branch of the unified "
                "/gen-env contract. It reuses existing pass_static_only and col"
                "lection evidence and does not claim policy evaluation."
            ),
        }
    )
    return record
    ...
    record.update(
        {
            "outputs": {
                "forge_fallback": {
                    ...
                    "import_gate": {
                        "status": "not_run",
                        "reason": (
                            "forge output is absent, so no RoboTwin/SAPIEN import, scale, c"
                            "ollision, support, material, or verifier gate can run"
                        ),
                    },
                },
                "handoff": {
            ...
            "claim_boundary": (
                "This sample validates the forge_fallback branch of the unified"
                " /gen-env contract. It is a blocker record and does not claim "
                "video2sim-forge execution."
            ),
        }
    )
    return record
    ...
    record.update(
        {
            "outputs": {
                "material_sidecar": {
                    ...
                    "render_gate": {
                        "status": "not_run",
                        "reason": (
                            "material sidecar output is absent, so renderer binding and vis"
                            "ual-material checks cannot run"
                        ),
                    },
                },
                "handoff": {
            ...
            "blockers": blocker["blockers"],
            "claim_boundary": (
                "This sample validates the material_sidecar branch of the unifi"
                "ed /gen-env contract. It is a blocker record and does not clai"
                "m NeuMaTeX execution."
            ),
        }
    )
    return record
...
def main() -> int:
    ...
    manifest = {
        ...
        "claim_boundary": (
            "Validates the unified /gen-env route contract over selection2e"
            "nv, forge fallback blocker, and material sidecar blocker examp"
            "les."
        ),
    }
