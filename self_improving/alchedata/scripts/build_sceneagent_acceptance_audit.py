...
def build() -> dict[str, Any]:
    ...
    require(
        not missing_selection_fields,
        f"Selection schema is missing fields: {sorted(missing_selection_fields)}",
    )
    require(
        {"task_binding", "placement_sha256", "verifier"}.issubset(
            task_program_schema.get("required", [])
        ),
    ...
    require(
        catalog.get("status") == "pass_unified_catalog_sources",
        "Unified catalog audit did not pass",
    )
    sources = catalog["sources"]
    require(
        sources["robotwin"]["execution_eligible_count"] == 123, "RoboTwin catalog count mismatch"
    )
    require(
        sources["agenticsim"]["execution_eligible_count"] == 8, "AgenticSim alias count mismatch"
    )
    require(sources["articraft10k"]["entry_count"] == 9996, "Articraft manifest count mismatch")
    require(
        sources["articraft10k"]["execution_eligible_count"] == 0,
        "Articraft default execution gate is unsafe",
    )
    ...
    for row in supported:
        ...
        require(
            ...
            f"{row['task_id']}: Base_Task smoke failed",
        )
        require(
            smoke.get("collect_dry_run", {}).get("status") == "pass_collect_dry_run",
            f"{row['task_id']}: /collect dry-run failed",
        )
    ...
    for path_value in final_collect_paths:
        ...
        require(
            report.get("status") == "pass_collect_dry_run", f"Fresh /collect failed: {path_value}"
        )
        video = probe_video(resolve_evidence_path(report["observer_video"]))
        require(
            video["frame_count"] == report["video_capture"]["frame_count"] == 97,
            f"Fresh /collect video mismatch: {path_value}",
        )
        require(
            report["video_capture"]["video_endpoint_only"] is False,
            f"Fresh /collect is endpoint-only: {path_value}",
        )
    ...
    require(
        set(visual.get("assessment_dimensions", [])) == REQUIRED_VISUAL_DIMENSIONS,
        "Visual review dimensions are incomplete",
    )
    ...
    for case in visual["cases"]:
        for dimension in REQUIRED_VISUAL_DIMENSIONS:
            require(
                str(case["checks"].get(dimension, "")).startswith("pass"),
                f"{case['task_id']}: failed visual dimension {dimension}",
            )
        require(
            case["checks"].get("final_task_relation") == "not_claimed",
            f"{case['task_id']}: initial review overclaims task success",
        )
        for image_name, path_value in case["images"].items():
            require(
                sha256_file(ROOT / path_value) == case["image_sha256"][image_name],
                f"{case['task_id']}: image hash mismatch",
            )
    ...
    require(
        accepted_candidates >= 6 and rejected_candidates > 0,
        "Accepted/rejected candidate evidence is incomplete",
    )
    source_lock = read_json(ROOT / "external_sources.lock.json")["sources"][
        "robotwin_text2env_demo"
    ]
    patch_path = ROOT / source_lock["patch"]
    require(
        sha256_file(patch_path) == source_lock["patch_sha256"], "Public-base patch hash mismatch"
    )
    ...
    require(
        fallback.get("status") == "blocked_fallback_inputs_missing",
        "Generation fallback summary is not a typed blocker",
    )
    ...
    alternate = read_json(
        ROOT / "artifacts/task_program_inputs/task_apple_plate_to_left_front.json"
    )
    ...
    require(
        ...
        "Same-scene execution failed",
    )
    require(
        decoupling.get("initial_observer_bytes_identical") is True,
        "Same-scene initial pixels differ",
    )
    for rollout in decoupling.get("rollouts", []):
        ...
        require(
            video["frame_count"] == rollout["video_capture"]["frame_count"],
            "Decoupling video frame mismatch",
        )
    ...
    audit_items = [
        item(
            ...
            {
                "required_selection_fields": sorted(EXPECTED_SELECTION_FIELDS),
                "task_program_binding_required": True,
            },
            [
                "schemas/selection2env.schema.json",
                "schemas/robotwin_task_program_input.schema.json",
        ...
        item(
            ...
            {
                "robotwin_execution_eligible": 123,
                "agenticsim_aliases": 8,
                "articraft_searchable": 9996,
                "articraft_default_execution_eligible": 0,
            },
            [
                "artifacts/adapter_catalog/selection2env_catalog_sources.json",
        ...
        item(
            ...
            {
                "supported_task_ids": [row["task_id"] for row in supported],
                "unsupported_task_ids": [row["task_id"] for row in unsupported],
                "fresh_collect": final_collect_results,
            },
        ...
        item(
            ...
            {
                "dimensions": sorted(REQUIRED_VISUAL_DIMENSIONS),
                "case_count": len(visual["cases"]),
                "final_task_relation": "not_claimed_in_initial_review",
            },
            [
                "artifacts/visual_review/selection2env_visual_review.json",
                "artifacts/visual_review/generated_rollout_final_relation_review.json",
            ],
        ),
        item(
            5,
            ...
            ),
            {
                ...
                "public_base": source_lock["base_commit"],
                "patch_sha256": source_lock["patch_sha256"],
        ...
        item(
            ...
            "generation2env blocker memo",
            {
                ...
                "typed_probe_count": len(fallback["probes"]),
                "completion_gate": "excluded_by_dashboard_acceptance",
            },
            [
                "docs/generation2env_blockers.md",
                "artifacts/generation_fallback/fallback_blocker_summary.json",
            ],
        ),
        item(
            7,
            ...
            {
                "commands": ["/collect", "/train", "/evaluate"],
                "supported_task_count": len(supported),
            },
            [
                "docs/selection2env_schema.md",
                "docs/openxsim_command_spec.md",
                "artifacts/openxsim/openxsim_command_registry.json",
            ],
        ),
        item(
            8,
