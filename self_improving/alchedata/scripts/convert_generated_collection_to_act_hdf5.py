...
def convert_episode(
    episode: dict[str, Any], output_path: Path, camera_size: tuple[int, int]
) -> dict[str, Any]:
    ...
    frames = np.stack(
        [initial if index < len(qpos) - 1 else final for index in range(len(qpos))], axis=0
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Convert generated play_once collection traces into ACT HDF5 smoke data."
    )
    ...
    collections = (
        [Path(item).expanduser().resolve() for item in args.collection]
        if args.collection
    ...
    for collection_path in collections:
        ...
        for episode in collection.get("episodes", []):
            converted = convert_episode(
                episode, out_dir / "data" / f"episode_{next_episode}.hdf5", camera_size
            )
    ...
    pass_count = sum(
        1 for episode in report["episodes"] if episode["status"] == "pass_act_hdf5_episode"
    )
    skip_count = sum(
    ...
    sim_task_config = {
        sim_task_name: {
            ...
            "episode_len": max(
                (episode.get("timesteps", 0) for episode in report["episodes"]), default=0
            ),
            "camera_names": ["cam_high"],
        }
    }
    report.update(
        {
            "status": "pass_act_hdf5_adapter_smoke"
            if pass_count >= 2 and fail_count == 0
            else "blocked_act_hdf5_adapter_smoke",
