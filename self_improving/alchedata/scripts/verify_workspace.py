...
def main() -> int:
    ...
    require(
        memory_ablation.get("status") == "pass_matched_memory_ablation",
        "memory ablation did not pass",
    )
    require(
    ...
    require(
        isaac_bundle.get("status") == "pass_isaac_command_bundle",
        "Isaac command bundle did not pass",
    )
    require(
        isaac_bundle.get("file_count") == len(isaac_bundle.get("files", [])) == 34,
        "Isaac bundle file count mismatch",
    )
    ...
    for row in isaac_bundle["files"]:
        ...
        require(
            ...
            f"Isaac bundle hash mismatch: {path}",
        )
    ...
    require(
        patch_sha256 == external_lock["patch_sha256"],
        "robotwin-text2env dependency patch hash mismatch",
    )
    external_repo = ROOT / "external" / "robotwin-text2env-demo"
    require(external_repo.exists(), "robotwin-text2env checkout missing")
    require(
        git_output(external_repo, "rev-parse", "HEAD^{tree}") == external_lock["result_tree"],
        "robotwin-text2env checkout tree mismatch",
    )
    require(
    ...
    require(
        git_output(
            external_repo, "merge-base", "--is-ancestor", external_lock["base_commit"], "HEAD"
        )
        == "",
    ...
    require(
        catalog_sources.get("robotwin", {}).get("entry_count", 0) >= 100,
        "RoboTwin discovered catalog too small",
    )
    require(
        catalog_sources.get("robotwin", {}).get("execution_eligible_count", 0) >= 100,
        "RoboTwin execution catalog too small",
    )
    require(
        catalog_sources.get("agenticsim", {}).get("source_commit") == "f34d56a",
        "unexpected AgenticSim source snapshot",
    )
    require(
        catalog_sources.get("agenticsim", {}).get("entry_count") == 8,
        "unexpected AgenticSim alias count",
    )
    require(
        catalog_sources.get("agenticsim", {}).get("execution_eligible_count") == 8,
        "AgenticSim aliases do not all resolve to RoboTwin",
    )
    for mapping in catalog_sources.get("agenticsim", {}).get("backend_mappings", []):
        require(
            mapping.get("selection_eligible") is True,
            f"AgenticSim backend mapping is blocked: {mapping.get('asset_id')}",
        )
        require(
            mapping.get("execution_status") == "robotwin_alias_loadable",
            f"AgenticSim mapping not loadable: {mapping.get('asset_id')}",
        )
    require(
        catalog_sources.get("articraft10k", {}).get("entry_count", 0) >= 9000,
        "Articraft catalog source too small",
    )
    require(
        catalog_sources.get("articraft10k", {}).get("execution_eligible_count") == 0,
        "Articraft metadata must not bypass import gates",
    )
    for query_path in catalog_source_audit.get("query_artifacts", []):
        query = read_json(artifact_path(query_path))
        require(
            {"robotwin", "agenticsim", "articraft10k"}.issubset(query.get("source_stats", {})),
            f"catalog query missing sources: {query_path}",
        )

    articraft_probe = read_json(ROOT / "artifacts" / "adapter_catalog" / "articraft10k_probe.json")
    require(
        articraft_probe.get("status") == "pass_hf_metadata_manifest",
        "unexpected Articraft probe status",
    )
    ...
    articraft_manifest = read_json(
        ROOT / "artifacts" / "adapter_catalog" / "articraft10k_manifest.json"
    )
    require(
        articraft_manifest.get("status") == "pass_searchable_manifest_from_hf_metadata",
        "unexpected Articraft manifest status",
    )
    require(
        articraft_manifest.get("asset_count", 0) >= 9000, "Articraft manifest asset count too low"
    )
    require(
        len(articraft_manifest.get("entries", [])) == articraft_manifest.get("asset_count"),
        "Articraft manifest entries/count mismatch",
    )
    require(
        articraft_manifest.get("asset_format_in_archive") == "URDF",
        "Articraft asset format must remain URDF",
    )
    for entry in articraft_manifest.get("entries", [])[:25]:
        require(
            entry.get("source_path", "").endswith(".tar.gz"), "Articraft entry missing archive path"
        )
        require(
            entry.get("download_url", "").startswith("https://huggingface.co/datasets/"),
            "Articraft entry missing Hub download URL",
        )
        require(entry.get("semantic_tokens"), "Articraft entry missing semantic tokens")
        require(
            entry.get("import_status") == "catalog_only_not_imported",
            "Articraft entry must not claim import",
        )
    articraft_search = read_json(
        ROOT / "artifacts" / "adapter_catalog" / "articraft10k_search_examples.json"
    )
    require(len(articraft_search.get("queries", [])) >= 5, "Articraft search examples missing")
    for query in articraft_search.get("queries", []):
        require(
            query.get("matches"), f"Articraft search query has no matches: {query.get('query')}"
        )
    ...
    for report, expected in ARTICRAFT_ARCHIVE_PROBES.items():
        ...
        require(
            archive_probe.get("status") == expected["status"],
            f"{report}: unexpected Articraft archive probe status",
        )
        selected_asset = archive_probe.get("selected_asset", {})
        require(
            selected_asset.get("asset_id") == expected["asset_id"],
            f"{report}: unexpected Articraft archive probe asset",
        )
        require(
            archive_probe.get("archive", {}).get("member_count", 0) >= 3,
            f"{report}: unexpected Articraft archive member count",
        )
        require(
            artifact_path(archive_probe.get("archive", {}).get("path", "")).exists(),
            f"{report}: missing Articraft downloaded archive",
        )
        urdf = archive_probe.get("urdf", {})
        require(
            artifact_path(urdf.get("path", "")).exists(),
            f"{report}: missing Articraft extracted URDF",
        )
        ...
        require(
            not urdf.get("missing_mesh_references"),
            f"{report}: Articraft URDF has unresolved mesh references",
        )
        ...
        for check_name in (
            ...
            "links_present",
            "joints_present",
            "mesh_references_resolved",
        ):
            require(
                checks.get(check_name) is True, f"{report}: Articraft check failed: {check_name}"
            )
        if expected["collision_gate"] == "pass":
            require(
                urdf.get("collision_geometry_count", 0) > 0,
                f"{report}: Articraft URDF missing collision geometry",
            )
            require(
                checks.get("collision_geometry_present") is True,
                f"{report}: Articraft collision check did not pass",
            )
        else:
            require(
                urdf.get("collision_geometry_count") == 0,
                f"{report}: expected collision-geometry blocker",
            )
            require(
                checks.get("collision_geometry_present") is False,
                f"{report}: expected failed collision-geometry check",
            )
        sapien_smoke = archive_probe.get("sapien_smoke", {})
        require(
            sapien_smoke.get("status") == "pass",
            f"{report}: Articraft SAPIEN smoke did not load/step",
        )
        require(
            sapien_smoke.get("articulation_type") == "PhysxArticulation",
            f"{report}: Articraft SAPIEN smoke did not load an articulation",
        )
        require(
            sapien_smoke.get("link_count") == urdf.get("link_count"),
            f"{report}: unexpected Articraft SAPIEN link count",
        )
        require(
            sapien_smoke.get("active_joint_count", 0) > 0,
            f"{report}: Articraft SAPIEN active joint count too low",
        )
        require(
            sapien_smoke.get("physics_steps", 0) >= 20,
            f"{report}: Articraft SAPIEN physics steps too low",
        )
    articraft_archive_passes = sum(
        1
        for report in articraft_archive_reports
        if report.get("status") == "pass_articraft_archive_sapien_smoke"
    )
    ...
    require(
        articraft_archive_passes >= 3, "expected at least three passing Articraft archive probes"
    )
    require(
        articraft_collision_blockers >= 2,
        "expected at least two Articraft collision-geometry blocker probes",
    )
    runtime = read_json(ROOT / "artifacts" / "runtime" / "robotwin_5090_env.json")
    require(
        runtime.get("torch_cuda_kernel_test") == "pass",
        "robotwin-5090 torch CUDA test did not pass",
    )
    require(runtime.get("curobo_install") == "pass", "robotwin-5090 CuRobo install did not pass")
    visual_review = read_json(
        ROOT / "artifacts" / "visual_review" / "selection2env_visual_review.json"
    )
    require(
        visual_review.get("overall_status")
        == "pass_initial_scene_support_with_task_success_pending",
    ...
    require(
        len(visual_review.get("cases", [])) >= 3,
        "visual review must cover the three supported tasks",
    )
    for case in visual_review.get("cases", []):
        checks = case.get("checks", {})
        require(
            checks.get("final_task_relation") == "not_claimed",
            f"{case.get('task_id')}: visual review must not claim final task success",
        )
        for dimension in required_visual_dimensions:
            require(
                str(checks.get(dimension, "")).startswith("pass"),
                f"{case.get('task_id')}: visual review dimension failed: {dimension}",
            )
        for image_name, image_path in case.get("images", {}).items():
            path = artifact_path(image_path)
            require(
                path.exists(), f"{case.get('task_id')}: missing visual review image {image_path}"
            )
            require(
                hashlib.sha256(path.read_bytes()).hexdigest()
                == case.get("image_sha256", {}).get(image_name),
    ...
    final_relation_review = read_json(
        ROOT / "artifacts" / "visual_review" / "generated_rollout_final_relation_review.json"
    )
    require(
        final_relation_review.get("overall_status")
        == "pass_scripted_final_relation_support_separate_fixed_scene_policy_eval_passed",
    ...
    require(
        {"task_apple_plate", "task_can_basket"}.issubset(reviewed_cases),
        "generated final-relation review must cover apple/plate and can/basket",
    )
    for task_id, case in reviewed_cases.items():
        require(
            case.get("metric_gate", {}).get("status") == "pass",
            f"{task_id}: final-relation metric gate did not pass",
        )
        require(
            case.get("metric_gate", {}).get("max_xy_distance_m", 999) < 0.08,
            f"{task_id}: final-relation xy metric too large",
        )
        require(
            len(case.get("episodes", [])) >= 3,
            f"{task_id}: final-relation review must cover three episodes",
        )
        for episode in case.get("episodes", []):
            require(
                episode.get("check_success") is True, f"{task_id}: reviewed episode did not pass"
            )
            require(
                episode.get("visual_relation_check") == "pass",
                f"{task_id}: visual relation check did not pass",
            )
            ...
            require(
                image_path.stat().st_size > 0, f"{task_id}: empty final observer image {image_path}"
            )

    action_repair_summary = read_json(
        ROOT
        / "artifacts"
        / "generated_rollout_repair"
        / "generated_selection2env_action_repair_summary.json"
    )
    require(
    ...
    require(
        len(action_repair_summary.get("passes", [])) >= 2,
        "generated action-repair summary must include both rollout passes",
    )
    require(
        len(action_repair_summary.get("collections", [])) >= 2,
        "generated action-repair summary must include both collections",
    )
    ...
    for run, expected_status in RUN_EXPECTATIONS.items():
        ...
        require(
            ...
            f"{run}: expected {expected_status}, got {summary.get('status')}",
        )
    ...
    for run, expected_status in SMOKE_EXPECTATIONS.items():
        ...
        require(
            ...
            f"{run}: expected {expected_status}, got {report.get('status')}",
        )
        ...
        for camera_name in ("observer_camera", "head_camera"):
            require(
                pixel_stats.get(camera_name, {}).get("std", 0) > 1,
                f"{run}: {camera_name} appears blank",
            )
    ...
    for run, expected_status in BASETASK_SMOKE_EXPECTATIONS.items():
        ...
        require(
            len(report.get("initial_poses", {})) >= 2,
            f"{run}: expected at least two loaded objects",
        )
        for image_path in report.get("images", {}).values():
            require(
                artifact_path(image_path).exists(),
                f"{run}: missing Base_Task smoke image {image_path}",
            )
    ...
    for run, expected_status in COLLECT_DRYRUN_EXPECTATIONS.items():
        ...
        require(report.get("object_count", 0) >= 2, f"{run}: expected at least two loaded objects")
        require(
            report.get("observation_file_count", 0) >= 4,
            f"{run}: expected saved camera observations",
        )
        require(
            report.get("policy_execution") == "not_run", f"{run}: must not claim policy execution"
        )
        require(
            report.get("task_success_claim") == "not_claimed", f"{run}: must not claim task success"
        )
    ...
    for run, expected_task_id in FINAL_ACCEPTANCE_COLLECT_EXPECTATIONS.items():
        ...
        require(
            report.get("status") == "pass_collect_dry_run",
            f"{run}: final acceptance /collect did not pass",
        )
        require(report.get("task_id") == expected_task_id, f"{run}: final acceptance task mismatch")
        video_capture = report.get("video_capture", {})
        require(
            video_capture.get("video_endpoint_only") is False,
            f"{run}: final acceptance video is endpoint-only",
        )
        require(
            video_capture.get("simulator_step_stride") == 1,
            f"{run}: final acceptance video is not per-step",
        )
        require(
            video_capture.get("frame_count") == 97, f"{run}: final acceptance frame count mismatch"
        )
        encoded_video = probe_video(artifact_path(report.get("observer_video", "")))
        require(
            encoded_video["frame_count"] == 97,
            f"{run}: encoded final acceptance frame count mismatch",
        )
        require(
            abs(encoded_video["fps"] - video_capture["fps"]) < 0.01,
            f"{run}: final acceptance fps mismatch",
        )
    ...
    for run, expected_status in OFFICIAL_ROLLOUT_EXPECTATIONS.items():
        ...
        require(
            report.get("probe_type") == "official_robotwin_play_once", f"{run}: wrong probe type"
        )
        require(
            report.get("check_success") is True, f"{run}: official task check_success did not pass"
        )
        require(report.get("move_event_count", 0) > 0, f"{run}: expected recorded move events")
        require(
            report.get("left_joint_path_len", 0) + report.get("right_joint_path_len", 0) > 0,
            f"{run}: expected planned joint paths",
        )
        diagnosis = report.get("failure_diagnosis", {})
        require(
            diagnosis.get("status") == "no_failure_observed", f"{run}: unexpected diagnosis status"
        )
        ...
        video_capture = report.get("video_capture", {})
        require(
            video_capture.get("endpoint_only") is False, f"{run}: observer video is endpoint-only"
        )
        require(
            report.get("video_endpoint_only") is False,
            f"{run}: endpoint-only compatibility flag is not false",
        )
        require(
            video_capture.get("frame_count", 0) >= 24, f"{run}: expected at least 24 real frames"
        )
        require(
            video_capture.get("duration_sec", 0) >= 2,
            f"{run}: expected at least 2 seconds of video",
        )
        ...
        require(
            ...
            f"{run}: encoded/report frame count mismatch",
        )
        require(
            abs(encoded_video["fps"] - video_capture["fps"]) < 0.01,
            f"{run}: encoded/report fps mismatch",
        )
        for image_path in report.get("images", {}).values():
            require(
                artifact_path(image_path).exists(), f"{run}: missing rollout image {image_path}"
            )
    ...
    for run, expected in GENERATED_ROLLOUT_EXPECTATIONS.items():
        ...
        report = read_json(report_path)
        require(
            report.get("status") == expected["status"],
            f"{run}: expected {expected['status']}, got {report.get('status')}",
        )
        require(
            report.get("probe_type") == "generated_selection2env_play_once",
            f"{run}: wrong probe type",
        )
        require(report.get("task_id") == expected["task_id"], f"{run}: wrong task id")
        require(
            report.get("plan_success") is True, f"{run}: generated play_once plan did not succeed"
        )
        require(
            report.get("check_success") is expected["check_success"],
            f"{run}: unexpected check_success",
        )
        require(report.get("move_event_count", 0) > 0, f"{run}: expected recorded move events")
        require(
            report.get("left_joint_path_len", 0) + report.get("right_joint_path_len", 0) > 0,
        ...
        require(
            metrics.get("xy_distance_m", 999) < 0.08,
            f"{run}: generated relation xy distance too large",
        )
        require(
            metrics.get("left_gripper_open") is True,
            f"{run}: left gripper should be open after release",
        )
        require(
            metrics.get("right_gripper_open") is True,
            f"{run}: right gripper should be open after release",
        )
        require(report.get("next_data_requirement"), f"{run}: missing next data requirement")
        for key in ("events", "move_events", "observer_video"):
            path = artifact_path(report.get(key, ""))
        ...
        for image_path in report.get("images", {}).values():
            require(
                artifact_path(image_path).exists(),
                f"{run}: missing generated rollout image {image_path}",
            )
    ...
    for run, expected in GENERATED_COLLECTION_EXPECTATIONS.items():
        ...
        require(
            report.get("probe_type")
            == "generated_selection2env_multi_episode_play_once_collection",
            f"{run}: wrong probe type",
        )
        require(report.get("task_id") == expected["task_id"], f"{run}: wrong task id")
        require(
            report.get("episode_count") == expected["episode_count"], f"{run}: wrong episode count"
        )
        require(
            report.get("pass_count") == expected["episode_count"],
            f"{run}: expected every episode to pass",
        )
        require(report.get("fail_count") == 0, f"{run}: expected zero failed episodes")
        require(
            report.get("policy_execution") == "generated_scripted_play_once",
            f"{run}: wrong policy execution boundary",
        )
        require(
            report.get("learned_policy_training") == "not_run", f"{run}: must not claim training"
        )
        require(
            report.get("learned_policy_evaluation") == "not_run",
            f"{run}: must not claim evaluation",
        )
        ...
        require(
            manifest.get("episode_count") == expected["episode_count"],
            f"{run}: manifest episode count mismatch",
        )
        for episode in report.get("episodes", []):
            require(
                episode.get("status") == "pass_generated_action_rollout",
                f"{run}: episode did not pass",
            )
            require(
                episode.get("check_success") is True, f"{run}: episode check_success did not pass"
            )
            require(
                episode.get("planned_joint_paths", 0) > 0,
                f"{run}: episode missing planned joint path",
            )
            require(
                episode.get("relation_metrics", {}).get("xy_distance_m", 999) < 0.08,
                f"{run}: episode relation xy distance too large",
            )
            for key in (
                "report",
                "events",
            ...
            for image_path in episode.get("images", {}).values():
                require(
                    artifact_path(image_path).exists(), f"{run}: missing episode image {image_path}"
                )

    act_conversion = read_json(ROOT / ACT_HDF5_CONVERSION_REPORT)
    require(
        act_conversion.get("status") == "pass_act_hdf5_adapter_smoke",
        "ACT HDF5 adapter smoke did not pass",
    )
    require(
        act_conversion.get("pass_count") >= 6,
        "ACT HDF5 adapter expected at least six converted episodes",
    )
    require(
        act_conversion.get("fail_count") == 0,
        "ACT HDF5 adapter should have zero hard conversion failures",
    )
    require(
        act_conversion.get("act_sim_task_config_json"),
        "ACT HDF5 adapter missing generated SIM_TASK_CONFIGS path",
    )
    require(
        artifact_path(act_conversion.get("act_sim_task_config_json", "")).exists(),
        "ACT HDF5 generated SIM_TASK_CONFIGS missing",
    )
    for episode in act_conversion.get("episodes", []):
        require(
            episode.get("status") == "pass_act_hdf5_episode",
            "ACT HDF5 adapter episode did not pass",
        )
    ...
    require(
        act_load.get("status") == "pass_act_hdf5_loader_smoke", "ACT HDF5 loader smoke did not pass"
    )
    ...
    require(
        act_eval.get("episode_count", 0) >= 3, "generated ACT eval expected at least three episodes"
    )
    require(
    ...
    require(
        act_eval.get("success_count", 0) + act_eval.get("failure_count", 0)
        == act_eval.get("episode_count"),
    ...
    require(
        len(eval_seeds) >= 3, "generated ACT eval expected at least three distinct held-out seeds"
    )
    require(
        eval_seeds.isdisjoint(source_training_seeds),
        "generated ACT eval seeds overlap source training seeds",
    )
    require(
        act_eval.get("all_eval_seeds_held_out") is True,
        "generated ACT eval did not mark all seeds held out",
    )
    require(
    ...
    require(
        act_eval.get("next_data_requirement"), "generated ACT eval missing next data requirement"
    )
    model_record = act_eval.get("model", {})
    require(
        model_record.get("all_keys_matched") is True,
        "generated ACT eval checkpoint keys did not match",
    )
    require(
        model_record.get("parameter_count", 0) > 0,
        "generated ACT eval missing model parameter count",
    )
    require(
    ...
    require(
        eval_checkpoint.stat().st_size > 0,
        f"generated ACT eval checkpoint empty: {eval_checkpoint}",
    )
    require(
    ...
    require(
        eval_run_state.get("command") == "/evaluate",
        "generated ACT eval run_state has wrong command",
    )
    require(
        eval_run_state.get("state") == "completed", "generated ACT eval run_state did not complete"
    )
    require(
    ...
    for episode in act_eval.get("episodes", []):
        require(
            episode.get("command") == "/evaluate", "generated ACT eval episode has wrong command"
        )
        require(
            episode.get("held_out_seed") is True, "generated ACT eval episode seed was not held out"
        )
        require(
            episode.get("execution_complete") is True, "generated ACT eval episode did not execute"
        )
        require(
            episode.get("policy_step_count", 0) >= 1,
            "generated ACT eval episode has no policy steps",
        )
        require(
            isinstance(episode.get("policy_success"), bool),
            "generated ACT eval episode missing policy verdict",
        )
    ...
    require(
        native_collection.get("episode_count") == 4,
        "native synchronized collection expected four attempts",
    )
    require(
        native_collection.get("pass_count") == 3,
        "native synchronized collection expected three task passes",
    )
    require(
        native_collection.get("fail_count") == 1,
        "native synchronized collection expected one retained task failure",
    )
    require(
    ...
    for episode in native_collection.get("episodes", []):
        ...
        require(
            native.get("status") == "pass_native_synchronized_recording",
            "native episode recording did not pass",
        )
    ...
    require(
        native_conversion.get("status") == "pass_native_act_hdf5_adapter",
        "native ACT conversion did not pass",
    )
    require(
        native_conversion.get("pass_count") == 3,
        "native ACT conversion expected three successful episodes",
    )
    require(
        native_conversion.get("skip_count") == 1,
        "native ACT conversion expected one skipped failed source",
    )
    ...
    for episode in native_conversion.get("episodes", []):
        if episode.get("status") != "pass_native_act_hdf5_episode":
            continue
        require(
            episode.get("act_timestep_count") == 161, "native ACT episode timestep count mismatch"
        )
        require(episode.get("action_dim") == 14, "native ACT episode action dimension mismatch")
        require(
            episode.get("camera_source") == "observation/head_camera/rgb",
            "native ACT camera source mismatch",
        )
        require(
            episode.get("native_jpeg_color_repair"), "native ACT episode missing JPEG repair marker"
        )
        path = artifact_path(episode.get("output", ""))
    ...
    require(
        native_load.get("status") == "pass_act_hdf5_loader_smoke", "native ACT loader did not pass"
    )
    ...
    require(
        native_replay.get("status") == "pass_act_action_replay_execution",
        "native ACT action replay did not execute",
    )
    require(
        native_replay.get("execution_complete") is True, "native ACT action replay did not complete"
    )
    require(
        native_replay.get("task_success") is True,
        "native ACT action replay did not satisfy the task verifier",
    )
    require(
        native_replay.get("executed_action_count") == 161, "native ACT action replay count mismatch"
    )
    require(
    ...
    require(
        native_replay.get("relation_metrics", {}).get("xy_distance_m", 999) < 0.13,
        "native replay relation failed",
    )
    ...
    require(
        native_train.get("status") == "pass_act_train_execution", "native ACT training did not pass"
    )
    ...
    require(
        native_train.get("best_val_loss", 999) < 0.02,
        "native ACT validation loss is above the recorded gate",
    )
    for key in ("policy_best", "dataset_stats", "loss_plot", "l1_plot", "kl_plot"):
    ...
    require(
        native_eval.get("status") == "pass_generated_act_evaluate_execution",
        "native ACT evaluation did not execute",
    )
    require(native_eval.get("episode_count") == 3, "native ACT evaluation expected three episodes")
    require(
        native_eval.get("execution_count") == 3,
        "native ACT evaluation did not execute all episodes",
    )
    require(
        native_eval.get("success_count") == 3,
        "native ACT evaluation did not pass all fixed-scene episodes",
    )
    require(
        native_eval.get("failure_count") == 0, "native ACT evaluation recorded unexpected failures"
    )
    require(
        native_eval.get("policy_success_rate") == 1.0, "native ACT evaluation success rate mismatch"
    )
    require(
        native_eval.get("all_eval_seeds_held_out") is True,
        "native ACT eval seeds overlap source seeds",
    )
    require(
        set(native_eval.get("held_out_seeds", [])) == {4, 5, 6},
        "native ACT held-out seed set mismatch",
    )
    require(
        set(native_eval.get("source_training_seeds", [])) == {0, 1, 2},
        "native ACT source seed set mismatch",
    )
    require(
        native_eval.get("camera_adapter", {}).get("runtime_source") == "head_camera",
        "native ACT eval camera mismatch",
    )
    require(
        native_eval.get("evaluation_scope", {}).get("action_selection")
        == "execute_full_161_action_chunk_before_replan",
        "native ACT eval action scheduling mismatch",
    )
    require(
        native_eval.get("evaluation_scope", {}).get("domain_randomization") is False,
        "native ACT eval claim boundary changed",
    )
    require(
        native_eval.get("model", {}).get("all_keys_matched") is True,
        "native ACT eval checkpoint keys did not match",
    )
    require(
        native_eval.get("model", {}).get("config", {}).get("chunk_size") == 161,
        "native ACT model chunk size mismatch",
    )
    ...
    require(
        native_checkpoint.stat().st_size > 0, f"native ACT checkpoint empty: {native_checkpoint}"
    )
    for key in ("events", "run_state"):
    ...
    for episode in native_eval.get("episodes", []):
        require(
            episode.get("execution_complete") is True, "native ACT eval episode did not complete"
        )
        require(episode.get("policy_success") is True, "native ACT eval episode did not pass")
        require(
            episode.get("policy_step_count") == 130, "native ACT eval episode success step changed"
        )
        diagnosis = read_json(artifact_path(episode.get("failure_diagnosis", "")))
        require(
            set(diagnosis.get("required_categories_checked", []))
            == REQUIRED_DENSE_DIAGNOSIS_CATEGORIES,
            "native ACT eval diagnosis category contract mismatch",
        )
    summary_eval = action_repair_summary.get("act_evaluate", {})
    require(
        summary_eval.get("status") == native_eval.get("status"),
        "action-repair summary eval status mismatch",
    )
    require(
    ...
    require(
        native_diagnosis.get("dataset_identity", {}).get("all_converted_episodes_byte_identical")
        is True,
        "native ACT diagnosis must retain byte-identical dataset evidence",
    )
    require(
        native_diagnosis.get("experiments", {})
        .get("chunk20", {})
        .get("evaluate", {})
        .get("success_count")
        == 0,
    ...
    require(
        native_diagnosis.get("conclusion", {}).get("policy_promotion")
        == "blocked_robustness_not_tested",
    ...
    require(
        initial_dataset.get("status") == "pass_act_dataset_diversity",
        "initial varied dataset gate did not pass",
    )
    require(
        initial_dataset.get("episode_count") == 12, "initial varied dataset episode count mismatch"
    )
    require(
        initial_dataset.get("unique_placement_signature_count") == 7,
        "initial varied placement count mismatch",
    )
    require(
        initial_dataset.get("unique_action_trajectory_count") == 11,
        "initial varied action count mismatch",
    )
    require(
        initial_iteration.get("train", {}).get("status") == "pass_act_train_execution",
        "initial varied ACT train did not pass",
    )
    ...
    require(
        initial_eval.get("all_eval_placements_held_out") is True,
        "initial eval placements are not held out",
    )
    ...
    require(
        recovery.get("all_failed_placements_recovered_as_expert_data") is True,
        "failed placements were not recovered",
    )
    require(
        recovery.get("recovery_collection", {}).get("pass_count") == 3,
        "recovery collection expected 3/3 passes",
    )
    recovery_dataset = recovery.get("dataset", {})
    require(
        recovery_dataset.get("status") == "pass_act_dataset_diversity",
        "recovery dataset gate did not pass",
    )
    require(recovery_dataset.get("episode_count") == 15, "recovery dataset episode count mismatch")
    require(
        recovery_dataset.get("unique_placement_signature_count") == 10,
        "recovery placement count mismatch",
    )
    require(
        recovery_dataset.get("unique_action_trajectory_count") == 14,
        "recovery action count mismatch",
    )
    require(
        recovery.get("train", {}).get("status") == "pass_act_train_execution",
        "recovery ACT train did not pass",
    )
    require(
    ...
    require(
        final_eval.get("all_eval_placements_held_out") is True,
        "new eval placements are not held out",
    )
    ...
    require(
        extra_task.get("native_synchronized_pass_count") == 2,
        "additional task native recording expected 2/2",
    )
    gates = robustness.get("promotion_gates", {})
    require(
        gates.get("new_heldout_eval_all_success") is False,
        "held-out success gate must remain rejected",
    )
    require(
        gates.get("additional_task_scripted_execution") is True,
        "additional task execution gate did not pass",
    )
    require(
        gates.get("declared_visual_physics_domain_randomization") is False,
        "domain-randomization gate is overstated",
    )
    require(
        gates.get("additional_task_learned_policy_evaluation") is False,
        "cross-task policy gate is overstated",
    )
    require(
    ...
    require(
        policy_probe.get("status") == "blocked_policy_train_eval_not_wired",
        "policy train/eval probe should remain an explicit blocker",
    )
    require(
        policy_probe.get("hdf5_file_count", 0) >= 6,
        "policy train/eval probe should see generated ACT HDF5 episodes",
    )
    require(
        policy_probe.get("act_hdf5_adapter", {}).get("conversion_status")
        == "pass_act_hdf5_adapter_smoke",
        "policy probe missing ACT HDF5 conversion pass",
    )
    require(
        policy_probe.get("act_hdf5_adapter", {}).get("loader_status")
        == "pass_act_hdf5_loader_smoke",
        "policy probe missing ACT HDF5 loader pass",
    )
    require(
        policy_probe.get("command_failure_count", 0) >= 3,
        "policy train/eval probe should record failed entrypoint checks",
    )
    blocking_reasons = " ".join(policy_probe.get("blocking_reasons", []))
    require(
        "entrypoint" in blocking_reasons, "policy train/eval probe must record entrypoint failures"
    )
    command_names = {item.get("name") for item in policy_probe.get("commands", [])}
    require(
        {"act_process_data", "act_train_entry", "act_eval_entry"}.issubset(command_names),
        "policy train/eval probe missing command coverage",
    )
    command_text = "\n".join(
    ...
    require(
        "No such file or directory" in command_text,
        "policy train/eval probe must record missing ACT data path",
    )
    require(
        "sim-task_apple_plate-demo_clean-3" in command_text,
        "policy train/eval probe must record missing default ACT task config",
    )
    require(
        "No module named 'envs.task_apple_plate'" in command_text,
        "policy train/eval probe must record eval task-env blocker",
    )
    for item in policy_probe.get("commands", []):
        require(
            artifact_path(item.get("stdout", "")).exists(),
            f"policy train/eval probe missing stdout log for {item.get('name')}",
        )
        require(
            artifact_path(item.get("stderr", "")).exists(),
            f"policy train/eval probe missing stderr log for {item.get('name')}",
        )
    ...
    require(
        openxsim_manifest.get("benchmark_count") == 3,
        "Open X Sim benchmark manifest must contain three bundles",
    )
    ...
    for row in benchmark_rows:
        ...
        require(
            source_rollout.get("status") == "pass_action_rollout",
            f"{task_name}: source rollout did not pass",
        )
        require(
            source_rollout.get("check_success") is True,
            f"{task_name}: source success verifier did not pass",
        )
        require(
            artifact_path(OPENXSIM_BENCHMARK_TASKS[task_name])
            == artifact_path(artifacts.get("rollout_report", "")),
            f"{task_name}: source rollout path mismatch",
        )

        run_state = read_json(artifact_path(artifacts.get("run_state", "")))
        require(
            run_state.get("status") == bundle.get("status"),
            f"{task_name}: run_state status mismatch",
        )
        require(
            run_state.get("current_stage") == "/evaluate",
            f"{task_name}: run_state did not reach /evaluate",
        )
        require(
            run_state.get("learned_policy") is False,
            f"{task_name}: run_state claim boundary mismatch",
        )
        require(
            run_state.get("next_data_requirement"),
            f"{task_name}: run_state missing next data requirement",
        )

        event_rows = [
            json.loads(line)
            for line in artifact_path(artifacts.get("events", ""))
        ...
        require(
            {event.get("command") for event in event_rows}.issuperset(
                {"/gen-env", "/collect", "/diagnose", "/evaluate"}
            ),
        ...
        require(
            scene_manifest.get("simulator_adapter") == "RoboTwin/SAPIEN",
            f"{task_name}: scene adapter mismatch",
        )
        require(
            len(scene_manifest.get("initial_entities", {})) >= 3,
            f"{task_name}: scene manifest missing entities",
        )
        require(
            len(scene_manifest.get("final_entities", {})) >= 3,
            f"{task_name}: final scene state missing entities",
        )
        require(
            len(scene_manifest.get("camera_artifacts", {})) >= 4,
            f"{task_name}: scene manifest missing cameras",
        )
        ...
        require(
            task_manifest.get("learned_policy") is False,
            f"{task_name}: task manifest claim boundary mismatch",
        )
        require(
        ...
        require(
            task_manifest.get("next_data_requirement"),
            f"{task_name}: task manifest missing next data requirement",
        )

        diagnosis = read_json(artifact_path(artifacts.get("failure_diagnosis", "")))
        require(
            diagnosis.get("status") == "no_failure_observed",
            f"{task_name}: benchmark diagnosis status mismatch",
        )
        require(
            REQUIRED_DENSE_DIAGNOSIS_CATEGORIES.issubset(
                set(diagnosis.get("checked_categories", []))
            ),
    ...
    require(
        {"selection2env", "forge_fallback", "material_sidecar"}.issubset(route_enum),
        "/gen-env schema missing route coverage",
    )
    ...
    for def_name, required_fields in branch_requirements.items():
        ...
        require(
            set(required_fields).issubset(set(definition.get("required", []))),
            f"/gen-env schema missing {def_name} required fields",
        )
    ...
    for report_path, expected in GEN_ENV_CONTRACT_EXAMPLES.items():
        ...
        require(
            expected["output_key"] in report.get("outputs", {}),
            f"{report_path}: missing route output branch",
        )
        ...
        if expected.get("required_blocker"):
            ...
            require(
                expected["required_blocker"] in blocker_codes,
                f"{report_path}: missing required blocker",
            )
    gen_env_contract_manifest = read_json(ROOT / GEN_ENV_CONTRACT_MANIFEST)
    require(
        gen_env_contract_manifest.get("status") == "pass_gen_env_contract_examples",
        "unexpected /gen-env contract manifest status",
    )
    require(
        set(gen_env_contract_manifest.get("examples", [])) == set(GEN_ENV_CONTRACT_EXAMPLES),
        "/gen-env contract manifest example list mismatch",
    )
    ...
    for report_path, expected in GEN_ENV_FALLBACK_PROBES.items():
        ...
        require(report.get("command") == "/gen-env", f"{report_path}: wrong command")
        require(
            report.get("fallback_type") == expected["fallback_type"],
            f"{report_path}: wrong fallback type",
        )
        require(report.get("status") == expected["status"], f"{report_path}: wrong fallback status")
        require(report.get("claim_boundary"), f"{report_path}: missing claim boundary")
        require(
            len(report.get("expected_outputs", [])) >= 5, f"{report_path}: missing expected outputs"
        )
        checks = report.get("checks", [])
        require(
            any(check.get("status") == "fail" for check in checks),
            f"{report_path}: expected at least one failed gate",
        )
        import_gates = report.get("import_gate_matrix", {})
        require(
            set(import_gates)
            == {"import", "scale", "collision", "support", "material", "render", "verifier"},
    ...
    require(
        fallback_summary.get("status") == "blocked_fallback_inputs_missing",
        "unexpected /gen-env fallback summary status",
    )
    require(
        set(fallback_summary.get("probes", [])) == set(GEN_ENV_FALLBACK_PROBES),
        "fallback summary probe list mismatch",
    )
    ...
    for artifact in artifacts:
        ...
        require(
            artifact.get("catalog_sources", {})
            .get("agenticsim", {})
            .get("selection_eligible_count")
            == 8,
            f"{artifact['task_id']}: AgenticSim aliases not connected",
        )
        require(
            artifact.get("catalog_sources", {}).get("articraft10k", {}).get("entry_count", 0)
            >= 9000,
        ...
        if artifact["status"] == "pass_sim_smoke":
            ...
            require(
                smoke.get("asset_load_render", {}).get("status") == "pass_asset_load_render",
                f"{artifact['task_id']}: missing asset-load render smoke",
            )
            require(
                smoke.get("basetask_curobo", {}).get("status") == "pass",
                f"{artifact['task_id']}: missing Base_Task/CuRobo smoke",
            )
            require(
                smoke.get("collect_dry_run", {}).get("status") == "pass_collect_dry_run",
                f"{artifact['task_id']}: missing /collect dry-run",
            )
            require(
                smoke.get("collect_dry_run", {}).get("policy_execution") == "not_run",
                f"{artifact['task_id']}: collect dry-run must not claim policy rollout",
            )
            require(
                "VISUAL_REVIEW_REQUIRED"
                not in {blocker.get("code") for blocker in artifact.get("blockers", [])},
    ...
    require(
        len(task_program_paths) >= 4, "expected primary task programs plus same-scene alternate"
    )
    ...
    for task_program_path in task_program_paths:
        ...
        require(
            reference_report["status"] == "pass",
            f"{task_program_path}: task-program references do not resolve",
        )
    ...
    require(
        "task_apple_plate_to_left_front" in task_programs,
        "same-scene alternate task-program input missing",
    )
    ...
    require(
        ...
        "same-scene task-program pair failed strict reference validation",
    )
    ...
    require(
        decoupling.get("task_count") == 2,
        "scene-task decoupling must contain exactly two task executions",
    )
    require(
    ...
    require(
        decoupling.get("scene_id") == pair_report.get("scene_id"), "decoupling scene_id mismatch"
    )
    require(
        decoupling.get("placement_sha256") == pair_report.get("placement_sha256"),
        "decoupling placement hash mismatch",
    )
    for rollout in decoupling.get("rollouts", []):
        require(
            rollout.get("status") == "pass", f"decoupling rollout failed: {rollout.get('task_id')}"
        )
        require(
            all(rollout.get("checks", {}).values()),
            f"decoupling rollout check failed: {rollout.get('task_id')}",
        )
        ...
        require(
            ...
            f"decoupling video is endpoint-only: {rollout.get('task_id')}",
        )
        require(
            video_capture.get("frame_count", 0) >= 500,
            f"decoupling video is too short: {rollout.get('task_id')}",
        )
    ...
    initial_observer_hashes = [
        hashlib.sha256(path.read_bytes()).hexdigest() for path in initial_observer_paths
    ]
    require(
        len(set(initial_observer_hashes)) == 1,
        "scene-task decoupling initial observer image hashes differ",
    )
    require(
    ...
    candidate_backend_ids = {
        candidate.get("backend_asset_id")
        for candidate in blocker_artifact.get("asset_candidates", [])
    }
    require(
        {"036_cabinet", "039_mug"}.issubset(candidate_backend_ids),
        "drawer/mug existing assets were not discovered",
    )
