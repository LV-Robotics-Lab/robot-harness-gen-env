    ...
    def interpret(self, bundle: InputBundle, *, output_root: Path, timeout: int = 600):
        ...
        try:
            ...
            prompt = (
                ...
                "For a rigid object with no requested moving joint, articulation_state must "
                "be null. A solid shape or an explicit absence of openings does not specify "
                "an articulation; never encode it as specified with an empty joint list. "
                "For structural_support with a declared color, also supply surface_rgba as "
                "an estimated uniform sRGB RGBA value in [0,1], and explain the estimate in "
                "color provenance. Preserve the original color description. This is not "
                "texture recovery. Foreground surface_rgba must be null; its material is "
                "resolved by asset tools. "
