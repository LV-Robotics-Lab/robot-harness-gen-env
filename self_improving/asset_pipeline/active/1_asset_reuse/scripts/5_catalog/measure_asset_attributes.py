...
def main():
    ...
    ap.add_argument("--catalog", default=str(ASSET_CATALOG))
    ap.add_argument("--shadow", default=str(ROBOTWIN_SHADOW_ROOT))
    ap.add_argument("--overrides", default=str(ASSET_OVERRIDES))
    ap.add_argument(
        "--out",
        default=str(ASSET_PIPELINE_ROOT / "data/scene_gen_ext/asset_attributes.json"),
    )
    ...
    _raw = yaml.safe_load(Path(a.overrides).read_text())
