...
> **协调更正（同日补记）：** 本文由并发恢复任务在不了解 consolidation 操作时写成，
> 因而把目录迁移和重复副本清理误判成“约 530GB 丢失”。核对后，301–361 资产库的
> 12,047 个文件（27,637,543,884 bytes）、317–361 账本、508 模型颜色结果、538 条
> 原点校准、471 条顶面/内腔探针、sweep/属性矩阵结果和 RoboTwin payload 都已迁到
> `/home/jingxiang/workspace/robot-harness-gen-env` 的 ignored runtime 目录，并有本地
> SHA-256 manifest；并非从磁盘消失。本文下方保留当时观察，作为为什么需要 Git 化
> 小型实测元数据、并在并发清理前协调活跃任务的历史记录，不再作为最终损失清单。
...
下表是 02:14 时恢复任务所见的临时判断；以页首协调更正和仓库
`self_improving/workspace_archives/20260814/` 的清理 receipt 为最终状态。
