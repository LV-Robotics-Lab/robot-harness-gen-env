...
本项目使用 `self_improving/onboarding/yeyuxuan/tools/migrate_robolab_asset.py`，将 RoboLab 中的单网格
...
conda run -n robolab-migrate \
  python -m pip install -r self_improving/onboarding/yeyuxuan/requirements-migrate.txt
```
...
```bash
conda run -n robolab-migrate \
  python self_improving/onboarding/yeyuxuan/tools/migrate_robolab_asset.py \
...
当前 22 资产结果见 [`robolab_migration_summary_22.md`](robolab_migration_summary_22.md)；
历史 21 与 20 资产快照仍分别保留在 [`robolab_migration_summary_21.md`](robolab_migration_summary_21.md)
和 [`robolab_migration_summary_20.md`](robolab_migration_summary_20.md)。
