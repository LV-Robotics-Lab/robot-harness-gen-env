...
**Initial Scene Reset（初始场景重置）**:
在开始测量前显式恢复已构建场景的初始状态；不证明经历运动后仍能恢复。
_Avoid_: 新建进程、步进后恢复证明

**Post-step Restoration（步进后恢复）**:
场景经历实际仿真变化后再次恢复约定初始状态的能力，须与仅初始重置分开验证。
_Avoid_: 初始零速度、重新构建场景
...
**Repair Reservation（修订额度预留）**:
对某一工作流状态和失败证据批准的修订额度；尝试失败或取消也已消耗，不因没有新版本而返还。
_Avoid_: 只累计成功次数、无成本重试
