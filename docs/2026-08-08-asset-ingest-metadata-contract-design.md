...
| 字段 | 类型 / 约束 | 必填 | 现状 |
|---|---|---|---|
| `schema_version` | const `"asset_ledger.v1"` | ✅ | 新增 |
| `asset_id` | str，IR 合法标识符（现状形如 `external_315_shears_m0`，不重构 id 体系） | ✅ | 已有 |
| `model_id` | int | ✅ | 新增（现在只隐含在 asset_id 后缀里） |
| `category` | str 非空 | ✅ | 已有 |
| `semantic_name` | str（对齐上游 catalog 同名字段） | ✅ | 新增 |
| `kind` | `rigid` \| `articulated` | ✅ | 新增（现在隐含在 tags 里） |
| `tags` | list[str] | ✅ | 已有 |
...
| 字段 | 类型 | 必填 | 现状 |
|---|---|---|---|
| `aliases` | list[str]，≥1 | ✅ | 从 fragment 收编 |
| `colors` | list[str] | 可空 | 从 fragment 收编 |
| `materials` | list[str] | 可空 | 新增（上游 catalog 有，外部资产缺） |
| `description` | str | 可选 | 新增；**无当前读者**，为线 D 语义检索预留——若嫌冗余可删，validator 不检查 |

### 3.3 `physical`（读者：solver 摆放 + 迁移编译；现有组扩展）

| 字段 | 类型 / 约束 | 必填 | 现状 |
|---|---|---|---|
| `mesh_bbox_m` | [x,y,z] 米 | ✅ | 已有 |
| `mesh_up_axis` | `Y` \| `Z` | ✅ | 已有 |
| `origin_convention` | str（如 `bottom-center`） | ✅ | 新增（现散在 representation.metadata.origin） |
| `scale_applied` | float | ✅ | 已有 |
| `size_resolution` | `{mode, actual_max_dim_m, scale, reference_max_dim_m, reference_assets[], verdict}` | ✅ | 已有 |
| `conventions.is_static` | bool | ✅ | 已有 |
| `conventions.z_policy` | str 枚举（上游取值） | ✅ | 已有 |
| `conventions.footprint_shape` | `circle` \| `box` | ✅ | 已有 |
| `conventions.stable_pose_id` | str | ✅ | **从 fragment 收编** |
| `conventions.stable_orientation_wxyz` | [w,x,y,z]，单位四元数（validator 查模长） | ✅ | **从 fragment 收编**（缺它=躺着生成） |
| `conventions.support_margin_m` / `support_spawn_clearance_m` | float | 可选（默认继承上游惯例） | 新增 |
| `conventions.inherited_from` | str \| null（参照资产 id；null=无先例，惯例未验证） | ✅ | 已有（`precedent`/`note` 规范化改名） |
| `mass_kg` | `{value: float\|null, status: known\|unknown, runtime_default_kg}` | ✅ | 已有 |
| `friction` | 同上结构 | ✅ | 新增（桥接设计表③要求标 unknown 不编造） |

### 3.4 `representations[]`（读者：openxsim transfer / enrich；迁移核心）

| 字段 | 类型 / 约束 | 必填 | 现状 |
|---|---|---|---|
| `format` | str（glb/obj/urdf/usd…） | ✅ | 已有 |
| `backend` | `sapien` \| `isaacsim` \| `portable` | ✅ | 已有 |
| `role` | `visual` \| `collision` \| `visual_and_collision` | ✅ | 已有 |
| `uri` | 存在的文件路径 | ✅ | 已有 |
| `sha256` / `size_bytes` | 64 hex / int≥0，validator 与磁盘核对 | ✅ | 已有（size 偶缺，补齐） |
| `metadata.derived_from` | str（转换血缘：源文件） | 转换产物必填 | 已有（自由键固化为约定名） |
| `metadata.converter` | str（如 `omni.kit.asset_converter@isaac-5.1`） | 转换产物必填 | 新增 |
| `metadata.conversion_params` | dict（rotated_z2y、缩放、凸分解方式等） | 转换产物必填 | 部分已有，收拢 |
...
| 字段 | 类型 | 现状 |
|---|---|---|
| `joint_names` / `joint_types` / `limits` | list（与 URDF 声明核对，s13a 已产出） | 已有 |
| `closed_qpos` / `open_qpos` | list[float] | 已有（对齐上游 catalog 同名） |
| `balance_gate` | `{free_joints_allowed: bool, measured_equilibrium: list\|null}` | 新增（s13b `--allow-free-joints` 实测平衡位落账） |

### 3.6 `source`（读者：治理 / 追溯 / 发布合规）

| 字段 | 类型 | 必填 | 现状 |
|---|---|---|---|
| `library` / `group` / `file` | str | ✅ | 已有 |
| `url` | str | 有则填 | 新增 |
| `license` | `{spdx: str\|null, status: declared\|unknown, terms_note: str}` | ✅（unknown 合法但必须显式） | 已有（自由文本 → 结构化） |
| `retrieved_at` | ISO 日期 | ✅ | 新增 |
| `source_manifest_path` | str（SOURCE_MANIFEST 哈希清单） | ✅ | 已有实践，入账 |
| `selection_evidence_path` / `import_matrix_path` | str \| null（线 D / 批量导入产物） | 有则填 | 新增（挂链接，不复制内容） |
...
| 字段 | 类型 / 取值 |
|---|---|
| `backend` | `sapien` \| `isaacsim` \| … |
| `check` | `settle` \| `joint_sweep` \| `runtime_load` \| `e2e` \| `admission_report` |
| `verdict` | `pass` \| `fail` |
| `date` | ISO 日期 |
| `report_path` | str（results/ 下的验证 JSON） |
| `thresholds` | dict，可选（位移/倾角门限等） |

约定：**append-only**，重跑追加不覆盖；`admission_report` 只记 s14 报告路径并标注「视图层、可逆」，准入结果不改变池层任何字段。

### 3.8 派生字段（validator 推导，**禁止手写**）

`usable: bool` + `missing: list[str]`——按 §3 必填表推导，语义对齐上游 catalog 的同名机制（外部资产与上游资产从此同一套「可用性」判据）。
