...
- 任务 2 仿真迁移：**引擎 openxsim 已搬入**（`shared/openxsim/`，详见其 README/UPSTREAM）；**env-gen→openxsim 桥接已实现**（`import_env_gen` + `import_environment` 自动分派，`resolved_scene` → openxsim IR，importer 落 `shared/openxsim/`）；已验证端到端可用（env-gen 场景经 openxsim `transfer` 编译到 Isaac 后端，网格资产缺 USD 时如实报 blocker，不静默降级）；⏳ 剩余：迁移一致性验证口径（L0-L4）。
