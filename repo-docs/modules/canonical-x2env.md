...
本地颜色执行器已能消费已预留批准，生成新资产并要求新预览与原视觉校验 MATCH；失败保留产物，
pending 意图不会因此被接受。该执行器仍待 Harness 编排、来源续点与最终完成门一起验收。

结构设计补全不等于恢复真实尺度。部署显式授权世界 XY/yaw 后，可复用已声明的桌面厚度/高度；
局部 on 高度由资产实测几何推导。原 critical 与用户已知数值仍保留，模型输出必须符合逐轴来源；
无图文本仅能做不依赖视觉估计的补全。完成门独立重算来源和值并比对实际编译 policy；
...
待修订处暂停；controller 新版本复核和剩余来源续行仍在实施。
