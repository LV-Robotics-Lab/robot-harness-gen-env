...
100%覆盖要求保留历史，不再阻断本阶段；实际覆盖缺口继续报告，不删除测试或扩大排除。
内部新增显式私有Terra路由，固定模型`openai/gpt-5.6-terra`与max，密钥只读私有文件进入子进程环境。
同一输出合同也提供给模型，不依赖服务严格输出扩展；本地严格校验继续拒绝错误。
真实受管理interpret44.72秒通过，仅为模型组件；四例端到端仍须独立运行，旧认证/格式失败不回填。
正常缺失尺寸/位置可按显式生成policy选择仿真设计；未知字段支持准确原entity ID或原scene序号，
不会仅因`red-block.dimensions`这类定位格式要求用户补尺寸。已知轴、冲突和未支持字段仍严格拒绝。
...
完成门进一步拒绝畸形evidence/transport/event结构并返回明确错误；独立producer公开攻击与回归111项通过，
不等于完整覆盖门或真实多实体case。
...
165项相关回归通过。随后v2 compile持久化support CAS refs，包复制最小项目核验源码和冻结断言；
独立Python可不读原workspace/CAS核验，相关103项通过。v2 audit环境需Pydantic/NumPy/SciPy/
