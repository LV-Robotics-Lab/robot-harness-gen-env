...
> 让场景能在 **SAPIEN 之外的仿真后端**运行。本阶段的**迁移引擎 = openxsim**（Open-X-Sim）：一套**后端中立 IR**（`EnvironmentPackage`）+ 多后端编译 + 跨模拟器一致性测试，已搬入 `openxsim/` 并跑通自带测试。**与 env-gen 的桥接（把 env-gen 的 `resolved_scene` 喂进 openxsim IR）是后续开发**，见 `../docs/design.md`。
...
## 2. openxsim 提供的 5 个工作流（`openxsim/scripts/openxsim.py`）
| 子命令 | 作用 |
|---|---|
| `text2env` | 纯文本编译进 IR |
...
| 路径 | 作用 |
|---|---|
| `openxsim/source/agenticsim/agenticsim/openxsim/` | 引擎源码：`ir.py`(中立 IR) `backends.py`(多后端编译) `robotwin.py`(RoboTwin 适配) `conformance.py`(L0-L4 一致性) `importers.py` `text2env.py` `pipeline.py` `assets.py` |
| `openxsim/scripts/openxsim.py` | CLI 入口（自定位 `source/agenticsim`） |
| `openxsim/configs/` `openxsim/tests/` | 配置 / 测试（36 passed） |
| `openxsim/third_party/MetaSim/` | vendored 多后端仿真框架（含 IsaacSim 工具）。**不入 git**，见 `openxsim/UPSTREAM.md` |
| `openxsim/deps/metasim_core/` | vendored 依赖。**不入 git** |
...
- **MetaSim**（vendored 多后端仿真框架，openxsim 靠它对接 Isaac 等；见 `openxsim/UPSTREAM.md`）。
- **conformance L0-L4**（跨模拟器一致性的分级评估）。

## 6. 坑与注意
- 跑 openxsim 需把 `source/agenticsim`、`deps/metasim_core`、`third_party/MetaSim` 加进 `PYTHONPATH`（见上）。
- `third_party/` 与 `deps/` **不入 git**（本地已存在、可跑）；**fresh clone 需按 `openxsim/UPSTREAM.md` 重新填充**才能跑。
- **openxsim 目前独立于 env-gen**（有自己的 text2env/IR，不读 env-gen 的 `resolved_scene.json`）；打通两者是本阶段待开发的桥接。

## 7. 来历 / 依赖
- **引擎来历**：从 `/home/jingxiang/workspace/openxsim-validation` 搬入（2026-08-02），详见 `openxsim/UPSTREAM.md`。
